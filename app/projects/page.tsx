...
const mlAnimationGallery = [
  { src: '/ml-animations/linreg_descent.webp', label: 'Linear regression gradient descent' },
  { src: '/ml-animations/lasso_l1_geometry.webp', label: 'Lasso and L1 geometry' },
  { src: '/ml-animations/knn_vote.webp', label: 'K-nearest neighbors voting' },
  { src: '/ml-animations/svm_margin.webp', label: 'Support vector machine margin' },
  { src: '/ml-animations/lda_qda_boundary.webp', label: 'LDA and QDA decision boundaries' },
  { src: '/ml-animations/forest_variance.webp', label: 'Random forest variance reduction' },
  { src: '/ml-animations/boosting_residuals.webp', label: 'Gradient boosting residuals' },
  { src: '/ml-animations/kmeans_lloyd.webp', label: 'K-means with Lloyd\'s algorithm' },
  { src: '/ml-animations/meanshift_ascent.webp', label: 'Mean shift density ascent' },
  { src: '/ml-animations/dbscan_reachability.webp', label: 'DBSCAN density reachability' },
  { src: '/ml-animations/gmm_em.webp', label: 'Gaussian mixture expectation-maximization' },
  { src: '/ml-animations/crp_seating.webp', label: 'Dirichlet process mixture' },
  { src: '/ml-animations/backprop_chain.webp', label: 'Backpropagation and the chain rule' },
  { src: '/ml-animations/conv2d_slide.webp', label: '2D convolution and weight sharing' },
  { src: '/ml-animations/autoencoder_pca.webp', label: 'Autoencoder and PCA subspaces' },
  { src: '/ml-animations/bptt_vanishing.webp', label: 'Backpropagation through time' },
  { src: '/ml-animations/attention_scaling.webp', label: 'Scaled dot-product attention' },
  { src: '/ml-animations/mf_factorization.webp', label: 'Matrix factorization bottleneck' },
  { src: '/ml-animations/mf_learning.webp', label: 'Matrix factorization learning' },
]
  ...
  return (
    <div>
      ...
      <Shell className="px-6 py-6 sm:px-8">
        <p className="mb-8 text-[14px] leading-relaxed text-[var(--muted)]">
        ...
        <div className="mb-5">
          <div className="flex flex-col gap-3 sm:flex-row">
            <label htmlFor="project-search" className="relative flex-1">
              ...
              <Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[var(--soft)]" aria-hidden="true" />
              <input
                id="project-search"
                type="search"
                value={searchQuery}
                ...
                className="w-full rounded-lg border border-[var(--line)] bg-[var(--card)] py-3 pl-12 pr-4 text-sm text-[var(--fg)] outline-none transition-colors placeholder:text-[var(--soft)] focus:border-purple-500"
              />
            </label>
            <button
              type="button"
              ...
              className="inline-flex items-center justify-center gap-2 rounded-lg border border-[var(--line)] bg-[var(--card)] px-4 py-3 text-sm font-medium text-[var(--muted)] transition-colors hover:border-[var(--soft)] hover:text-[var(--fg)]"
            >
                ...
                <button
                  ...
                  className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                    activeField === field
                      ...
                      : 'border-[var(--line)] bg-[var(--card)] text-[var(--muted)] hover:border-[var(--soft)] hover:text-[var(--fg)]'
                  }`}
                >
                  {field}
                </button>
        ...
        <div className="mb-12 flex items-center gap-3">
          <a
            href="/opensource"
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-[var(--line)] bg-[var(--card)] text-[var(--muted)] text-sm font-medium transition-all duration-200 hover:border-[var(--soft)] hover:text-[var(--fg)]"
          >
        ...
        {projectSections.length > 0 ? (
                ...
                <div className="mb-6 flex items-end justify-between gap-4 border-b border-[var(--line)] pb-3">
                  <div>
                    <h2
                      ...
                      className="font-serif text-2xl tracking-tight text-[var(--fg)]"
                    >
                      {section.field}
                    </h2>
          ...
          <div className="rounded-lg border border-[var(--line)] bg-[var(--card)] p-8 text-center text-[var(--muted)]">
